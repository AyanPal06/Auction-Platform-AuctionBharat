...
export const addNewAuctionItem = catchAsyncErrors(async(req,res,next)=>{
    if (!req.files || Object.keys(req.files).length === 0) {
        return next(new ErrorHandeler("Auction item image required.", 400));
      }
      ...
      if(new Date(startTime)<Date.now()){
        return next (new ErrorHandeler("Auction starting time must be greater than present time.",400));
      }
      if(new Date(startTime) >= new Date(endTime)){
        return next (new ErrorHandeler("Auction starting time must be less than ending time.",400));
      }
      ...
      if(alreadyOneAuctionActive.length>0){
        return next(new ErrorHandeler("You already have one active auction .",400));
      }
      try{
        const cloudinaryResponse = await cloudinary.uploader.upload(image.tempFilePath,{
         ...
         if(!cloudinaryResponse || cloudinaryResponse.error){
          ...
          return next(new ErrorHandeler("Failed to upload auction image to cloudinary.",500));
         }
        ...
        return next(new ErrorHandeler(error.message || "Failed to create auction.", 500));
      } 
});
...
export const republishItem= catchAsyncErrors(async(req,res,next)=>{
  ...
  if(!req.body.startTime || !req.body.endTime){
    return next(new ErrorHandeler("StartTime and EndTime for republish is mendetory."));
  }
  if(new Date(auctionItem.endTime) > Date.now()){
    return next( new ErrorHandeler("Auction is already active,cannot republish.",400));
  }
  let data ={
