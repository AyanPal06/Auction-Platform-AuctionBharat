...
const Card = ({imgSrc, title, startingBid, startTime, endTime, id}) => {
    ...
    return (
    <>
      <Link to={`/auction/item/${id}`} className='flex-grow basis-full bg-white rounded-md groupsm:basis-56 lg:basis-60 2xl:basis-80'>
        ...
        <div className='px-2 pt-4 pb-2'>
            ...
            {
                startingBid && (
                    <p className='text-stone-600 font-light'>Starting Bid:{" "} <span className='text-[#fdba88] ml-1 font-bold'>{startingBid}</span></p>
                )
            }
            <p className='text-stone-600 font-light'>{timeLeft.type}
                {
