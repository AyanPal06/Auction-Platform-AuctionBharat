    ...
    <>
      <section className="w-full ml-0 m-0 h-fit px-5 pt-20 lg:pl-[320px] flex flex-col">
        <div className="text-[16px] flex flex-wrap gap-2 items-center ">
          <Link
            to="/"
            className="font-semibold transition-all duration-300 hover:text-[#d6842b]"
          >
            Home
          </Link>
        ...
        {loading ? (
          ...
          <div className="flex gap-4 flex-col lg:flex-row">
            <div className="flex-1 flex flex-col gap-3">
              ...
              <p className="txt-xl w-fit font-bold">Auction Item Description</p>
            ...
            <div className="flex-1">
              <header className="bg-stone-200 text-slate-700 py-4 text-[24px] font-semibold px-4">
                Bids
              </header>
                ...
                Date.now() <= new Date(auctionDetail.endTime) ? (
                  <>
                    <div className="flex gap-3 flex-col sm:flex-row sm:items-center">
                      ...
                      <input
                        type="number"
                        className="w-32 rounded-sm focus:outline-none md:text-[20px] p-1"
                        value={amount}
                        onChange={(e) => setAmount(e.target.value)}
                      />
                    </div>
                    <button className="p-4 text-slate-300 bg-slate-800 rounded-full transition-all duration-300 hover:bg-slate-700" onClick={handleBid}>
                        <RiAuctionFill/>  
                    </button>
                  </>
                ) : new Date(auctionDetail.startTime)>Date.now() ?(
